...
if ($karyawan_id) {
    ...
    $sqlD = "SELECT a.*, s.nama as shift_nama, s.jam_masuk, s.jam_keluar, l.nama as lokasi_nama
        ...
        WHERE a.karyawan_id=? AND {$date_cond} ORDER BY a.tanggal ASC";
...
.pct-bar-wrap { display:inline-block; width:36px; height:5px; background:#e5e7eb; border-radius:3px; vertical-align:middle; margin-left:3px; }
...
<div class="judul-wrap">
    ...
    <div class="periode">
        ...
        &mdash; Hari Kerja (Senin–Jumat): <strong><?= $jumlahHariKerja ?> hari</strong>
    </div>
...
<thead>
    <tr class="th-main">
        ...
        <?php if (!$departemen_id): ?><td class="left" style="width:60px">Departemen</td><?php endif; ?>
        <td style="width:30px">Hadir</td>
        <td style="width:30px">Telat</td>
        <td style="width:30px">Plg.</td>
        <td style="width:24px">Alpha</td>
        <td style="width:22px">Izin</td>
        <td style="width:24px">Sakit</td>
        <td style="width:56px">Total Telat</td>
        <td style="width:56px">Total Plg.</td>
        <td style="width:38px">Total Jam</td>
        <td style="width:40px">Kehadiran</td>
    </tr>
    <tr class="th-sub">
        <td colspan="<?= $departemen_id ? 3 : 4 ?>" class="left" style="color:#555;font-style:italic">Hari kerja: <?= $jumlahHariKerja ?> hari</td>
        <td colspan="10" style="color:#555;font-style:italic">Rekapitulasi <?= htmlspecialchars($period_label) ?><?= $namaDepartemen?' — Dept. '.htmlspecialchars($namaDepartemen):'' ?></td>
    </tr>
</thead>
<tbody>
<?php if(empty($rows)): ?>
...
$prevDept = null; $no = 0;
foreach($rows as $r):
    // Group header per departemen (hanya jika tampil semua departemen)
...
<tr>
    ...
    <?php if (!$departemen_id): ?><td style="font-size:7.5pt;color:#555"><?= htmlspecialchars($r['departemen_nama']??'-') ?></td><?php endif; ?>
...
?>
<tr class="total-row">
    <td colspan="<?= $departemen_id ? 3 : 4 ?>">TOTAL / RATA-RATA (<?= count($rows) ?> karyawan)</td>
    ...
    <td class="center" style="color:<?= $totalPct>=90?'#16a34a':($totalPct>=75?'#d97706':'#dc2626') ?>;font-weight:700"><?= $totalPct ?>%</td>
</tr>
<?php endif; ?>
</tbody>
</table>
...
<tr>
    <td style="width:55%">
        <div style="font-size:7.5pt;color:#555;line-height:1.7">
            ...
            3. Pulang cepat dihitung dari jam keluar shift setelah toleransi<br>
            4. Periode laporan: <strong><?= htmlspecialchars($period_label) ?></strong>
            <?php if ($namaDepartemen): ?><br>5. Departemen: <strong><?= htmlspecialchars($namaDepartemen) ?></strong><?php endif; ?>
        </div>
    </td>
    <td style="text-align:right;vertical-align:bottom">
        <div style="font-size:7.5pt;color:#333;margin-bottom:4px"><?= htmlspecialchars($namaPerusahaan) ?>, <?= date('d ') . ($mode_filter==='periode'?date('F Y',strtotime($tgl_sampai)):$monthsId[$bulan].' '.$tahun) ?></div>
